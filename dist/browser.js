...
(function (factory) {
    ...
    function initializeDynamicRowGroup(groupName, container) {
        ...
        var removeButtons = container.querySelectorAll('[data-cd-remove-row]');
    ...
    function handleRemoveRow(event) {
        ...
        var rowInfo = findRowContainingElement(group, button);
        if (!rowInfo)
            return;
        removeRow(group, rowInfo.row);
    }
    function addNewRow(group) {
        console.log("\u2795 Adding new row to group \"".concat(group.groupName, "\" (currently ").concat(group.rows.length, " rows)"));
        // Clone the template
        ...
        var removeButtons = newRow.querySelectorAll('[data-cd-remove-row]');
        console.log("\u2795 Attaching ".concat(removeButtons.length, " remove button listeners to new row"));
        removeButtons.forEach(function (removeButton) {
            removeButton.addEventListener('click', handleRemoveRow);
        });
        // Reindex all rows
        ...
        // Update summaries
        updateSummaries(group);
        console.log("\u2795 Row added successfully, new total: ".concat(group.rows.length, " rows"));
        // Dispatch event
    ...
    function updateSummaries(group) {
        var _a;
        console.log("\uD83D\uDCCA Updating summaries for group \"".concat(group.groupName, "\" (").concat(group.rows.length, " rows)"));
        ...
        console.log("\uD83D\uDCCA Found ".concat(summaryContainers.length, " summary container(s) for group \"").concat(group.groupName, "\""));
        summaryContainers.forEach(function (summaryContainer, containerIndex) {
            var template = summaryContainer.querySelector('[data-summary-template]');
            if (!template) {
                console.warn("\uD83D\uDCCA No template found in summary container ".concat(containerIndex, " for group \"").concat(group.groupName, "\""));
                return;
            }
            // Remove existing summary rows
            var existingSummaryRows = summaryContainer.querySelectorAll('[data-summary-row]');
            console.log("\uD83D\uDCCA Removing ".concat(existingSummaryRows.length, " existing summary rows from container ").concat(containerIndex));
            existingSummaryRows.forEach(function (row) { return row.remove(); });
            // Create summary rows for each data row
            console.log("\uD83D\uDCCA Creating ".concat(group.rows.length, " new summary rows for container ").concat(containerIndex));
            group.rows.forEach(function (dataRow, index) {
                ...
                console.log("\uD83D\uDCCA Processing ".concat(fieldElements.length, " field elements in summary row ").concat(rowIndex));
                fieldElements.forEach(function (element) {
                    ...
                    if (fieldPattern) {
                        ...
                        console.log("\uD83D\uDCCA Updated field: ".concat(fieldPattern, " \u2192 ").concat(finalFieldName));
                    }
                });
    ...
    function findRowContainingElement(group, element) {
    ...
    function removeRow(group, targetRow) {
        console.log("\u2796 Removing row from group \"".concat(group.groupName, "\" (currently ").concat(group.rows.length, " rows)"));
        ...
        var targetIndex = group.rows.indexOf(targetRow);
        if (targetIndex === -1) {
            console.warn('Target row not found in group:', group.groupName);
            return;
        }
        console.log("\u2796 Removing row at index ".concat(targetIndex));
        ...
        // Update summaries
        updateSummaries(group);
        console.log("\u2796 Row removed successfully, new total: ".concat(group.rows.length, " rows"));
        // Dispatch event
